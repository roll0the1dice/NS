...
ABountyDashHUD::ABountyDashHUD() {
	...
	static ConstructorHelpers::FObjectFinder<UFont> OurHudFont(TEXT("/Game/Barrel_Hopper/Font/AGENCYB_Font.AGENCYB_Font"));
...
void ABountyDashHUD::DrawHUD() {
	...
	if (DashGameMode->GetGameOver()) { 
		FVector2D GameOverSize; GetTextSize(TEXT("GAME OVER!!! Press R to Restart!"), GameOverSize.X, GameOverSize.Y, HUDFont); 
